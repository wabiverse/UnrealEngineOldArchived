...
static void VerifyMetalCompiler()
{
	...
	// CommandLineTools is no good for us as the Metal compiler isn't included
	{
		...
		if(!bFoundXCode)
		{
			FMessageDialog::Open(EAppMsgType::Ok, FText(NSLOCTEXT("AGXRHI", "XCodeMissingInstall", "Can't find Xcode install for Metal compiler. Please install Xcode and run Xcode-beta.app to accept license or ensure active developer directory is set to current Xcode installation using xcode-select.")));
			FPlatformMisc::RequestExit(true);
			return;
		}
	}
